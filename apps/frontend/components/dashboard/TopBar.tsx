...
export default function TopBar({ onOpenMobileMenu }: TopBarProps) {
  ...
  // Sync wallet address from Privy on mount or login
  useEffect(() => {
    if (user?.wallet?.address) {
      setWallet(user.wallet.address, user.email?.address || user.id);
    }
  }, [user, setWallet]);

  // Fetch backend profile data for this user address
  useEffect(() => {
    if (!address) return;
  ...
  return (
    <header className="flex h-20 items-center justify-between border-b border-slate-800 bg-[#0d112d]/50 px-6 backdrop-blur-md">
      ...
      {/* Right section: Wallet, balance and profile details */}
      <div className="flex items-center gap-4">
        {/* Mock Gasless stablecoin balance */}
